...
namespace RizzGamingBase.Models.Infra
{
    public class UploadFileHelper
    {
        ...
        public void UploadFile(HttpPostedFileBase file, string categoryFolderName, string[] allowedExtensions)
        {
            ...
            // 使用紅利商品的ID創建文件夾路徑，可自訂命名規則
            string bonusProductTypePath = Path.Combine(categoryFolderPath);
        ...
        public void DeleteFile(string categoryFolderName, string fileName)
        {
            var fullPath = HostingEnvironment.MapPath($"/Images/{categoryFolderName}/{fileName}");
            File.Delete(fullPath);
        }
